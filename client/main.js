...
Template.body.events({
  ...
  'click #organize': function(event,template){
  	...
  	document.querySelector('#words').innerHTML = '<header> <form class="protests"> <input type="text" class="date" placeholder="enter the date" /> <input type="text" class="location" placeholder="enter the location" /> <input type="text" class="cause" placeholder="enter the cause" /> <button type="submit"> Create </button> </form> </header> <ul> {{#each todos}} {{> info}} {{/each}} </ul> <template name="info"> <li>Protest at {{location}} on {{date}} for {{cause}}</li> </template> ';
  },
});
